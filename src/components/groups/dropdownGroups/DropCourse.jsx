...
export default function DropDisciplines({ onChange }) {
    ...
    useEffect(() => {
        axios.get('http://localhost:5008/api/Disciplines')
            .then(response => {
                ...
                setSelected(response.data.items[0].id);
            })
            .catch(error => {
    ...
    return (
        <Listbox value={selected} onChange={(selected) => { setSelected(selected.id); }}>
            {({ open }) => (
                <>
                        ...
                        <div className="relative mt-2">
                            <Listbox.Button className="relative w-full cursor-default rounded-md bg-white py-2.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
                                <span className="flex items-center">
                                    <span className="ml-3 block truncate">{disciplines.find(discipline => discipline.id === selected)?.name || "Loading..."}</span>
                                </span>
